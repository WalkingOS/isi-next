...
const IndexPage = async () => {
	const services = await sanityClient.fetch(
		'*[_type == "service" && "Esthetics" in categories[]->title] {title, description, slug}'
	);

	const category = await sanityClient.fetch(
		`*[_type == "category" && title=="Esthetics"][0] {title, subtitle, descriptionTitle, description, mainImage}`
	);
